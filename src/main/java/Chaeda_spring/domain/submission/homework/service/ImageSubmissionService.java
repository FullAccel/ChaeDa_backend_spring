...
public class ImageSubmissionService {
    ...
    public List<ImageResponse> uploadImageCompleteForSubmission(Long studentId, Long HwAnnouncementId, List<UploadImageCompleteRequest> requests) {
        ...
        for (UploadImageCompleteRequest request : requests) {
            ...
            if (request.imageType().equals(ImageType.HOMEWORK_SUBMISSION)) {
                SubmissionHomework submissionHomework = submissionHWRepository.findByHwAnnouncementIdAndStudentId(HwAnnouncementId, studentId);
                submissionHomework.completeHomework();
                submissionHomework.getHwAnnouncement().plusSubmissionNum();
                savedImage.setSubmissionHomework(submissionHomework);
            } else
    ...
    public List<ImageResponse> getSubmissionImages(Long studentId, Long hwAnnouncementId) {
        ...
        if (!submissionHomework.isCompleteStatus())
            throw new NotFoundException(ErrorCode.HOMEWORK_NOT_COMPLETE);

        return submissionHomework.getImageList().stream()
